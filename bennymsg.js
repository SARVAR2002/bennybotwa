...
const processTime = require('./utils/index')
...
module.exports = bennymsg = async (benny, message) => {
    try {
	...
	case '#sticknobg':
		...
		if (isMedia) {
                try {
                    ...
                    var result = await removeBackgroundFromImageBase64({ base64img, apiKey: 'vgLgNMf2qSSmR562jBA1N1g2', size: 'auto', type: 'auto', outFile })
    ...
    case '#quotes':
		if (isBanned) return await benny.reply(from, `Maaf nomor kamu telah dibanned!`, id)
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`)
            const quotes = await get.get('https://st4rz.herokuapp.com/api/randomquotes').json()
            benny.reply(from, `➸ *Quotes* : ${quotes.quotes}\n➸ *Author* : ${quotes.author}`, id)
            break
	case '#quotesnime':
		if (isBanned) return await benny.reply(from, `Maaf nomor kamu telah dibanned!`, id)
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`)
            const skya = await get.get('https://st4rz.herokuapp.com/api/quotesnime/random').json()
            skya_ = skya.data
    ...
    case '#randomnekonime':
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`)
            const nekonime = await get.get('https://st4rz.herokuapp.com/api/nekonime').json()
	...
	case '#tutorial':
		if (isBanned) return await benny.reply(from, `Maaf nomor kamu telah dibanned!`, id)
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`)
		    benny.sendLinkWithAutoPreview(from, `https://www.youtube.com/watch?v=sRm-5CExOCs`)
		    break
		case '#bot':
		if (isBanned) return await benny.reply(from, `Maaf, Nomor kamu telah dibanned!`, id)
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`)
    ...
    case '#wiki':
        ...
        if (args.length === 1) return benny.reply(from, 'Kirim perintah *#wiki [query]*\nContoh : *#wiki asu*', id)
            ...
            const wiki = await get.get('https://st4rz.herokuapp.com/api/wiki?q='+ query_).json()
            if (wiki.error) {
	...
	case '#igstalk':
		...
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`)
            ...
            const stalk = await get.get('https://st4rz.herokuapp.com/api/stalk?username='+ args[1]).json()
    ...
    case '#anime':
		...
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`)
            ...
            const animek = await get.get('https://st4rz.herokuapp.com/api/kuso?q=' + body.slice(7)).json()
	...
	case '#anime2':
		...
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`)
            if (args.length === 1) return benny.reply(from, 'Kirim perintah *#anime [query]*\nContoh : *#anime darling in the franxx*', id)
            const animeku = await get.get('https://st4rz.herokuapp.com/api/dewabatch?q=' + body.slice(7)).json()
	...
	if (isBanned) return await benny.reply(from, `Maaf nomor kamu telah dibanned!`, id)
            ...
            .then((result) => {
                ...
                .then((res) => {
                    ...
                    benny.reply(from, `Title: ${res.title}\n\nLink:\n${heheq}\nmasih tester bntr :v`)
                })
            })
            .catch(() => {
                benny.reply(from, 'Ada yang Error!', id)
	...
	case '#chord':
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`)
            ...
            const chord = await get.get('https://st4rz.herokuapp.com/api/chord?q='+ query__).json()
            if (chord.error) return benny.reply(from, chord.error, id)
            benny.reply(from, chord.result, id)
            break
    case '#listdaerah':
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`)
            const listDaerah = await get('https://st4rz.herokuapp.com/api/daerah').json()
	...
	case '#cooltext':
	    if (isBanned) return await benny.reply(from, `Maaf nomor kamu telah dibanned!`, id)
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`)
            if (args.length == 0) return benny.reply(from, `Untuk membuat teks keren CoolText pada gambar, gunakan ${prefix}cooltext teks\n\nContoh: ${prefix}cooltext arugaz`, id)
