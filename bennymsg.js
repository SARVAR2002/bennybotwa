...
module.exports = bennymsg = async (benny = new Client(), message) => {
		   ...
		   break
            ...
            case '#sfire':
		        ...
		        if (isMedia && type === 'image' || isQuotedImage) {
                    ...
                    await benny.reply(from, `Format salah!`, err)
                }
            break
	case '#stickernobg':
    case '#stikernobg':
		...
		if (!isRegistered) return await benny.reply(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`, id)
            ...
            if (arg.length >= 4) {
                ...
                await quotemaker(quotesa, authors, themea).then(amsue => {
                    benny.sendImageAsSticker(from, amsue, 'quotesmaker.jpg','neh...').catch(() => {
                       benny.reply(from, mess.error.Qm, id)
                    })
                })
            } else {
	...
	case '#bokep':
        ...
        case '#bkp':
          ...
          benny.sendFileFromUrl(from, rindBkep.image, 'Bokep.jpg', rindBkep.teks, id)
            break
...
${desc}`)
	...
	case '#jadikanowner':	
		if (!isSuper) return await benny.reply(from, `Perintah ini hanya bisa digunakan oleh super owner bot`, id)
		 ...
		 await benny.reply(from, `Success menambahkan dia menjadi owner bot!`, id)
            break
	case '#superowner':
	    if (!isSuper) return await benny.reply(from, `Perintah ini hanya bisa digunakan oleh super owner bot`, id)
	...
	case '#promoteme':
	    ...
	    if (!isSuper) return await benny.reply(from, `Perintah ini hanya bisa digunakan oleh super owner bot`, id)
        if (!isBotGroupAdmins) return benny.reply(from, 'Fitur ini hanya bisa di gunakan ketika bot menjadi admin', id)
            await benny.promoteParticipant(groupId, pilotNumber[1])
            await benny.sendTextWithMentions(from, `Perintah diterima, menambahkan @${pilotNumber} sebagai admin.`)
            break
	case '#unadminme':
	case '#demoteme':
		if (!isRegistered) return await benny.reply(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`, id)
	    ...
	    if (!isSuper) return await benny.reply(from, `Perintah ini hanya bisa digunakan oleh super owner bot`, id)
        if (!isBotGroupAdmins) return benny.reply(from, 'Fitur ini hanya bisa di gunakan ketika bot menjadi admin', id)
            await benny.demoteParticipant(groupId, pilotNumber[1])
	...
	case '#creategroup':
	    if (!isSuper) return await benny.reply(from, `Perintah ini hanya bisa digunakan oleh super owner bot`, id)
            arg = body.trim().split(' ')
            const gcname = arg[1]
            benny.createGroup(gcname, pilotNumber[0])
	...
	case '#culikdia':
            ...
            benny.sendText(from, 'Group Created ✨️')
            break
	case '#chatowner':
	    if (!isSuper) return await benny.reply(from, `Perintah ini hanya bisa digunakan oleh super owner bot`, id)
		if (!isRegistered) return await benny.reply(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`, id)
	...
	case '#report':
                ...
                if (lastReport !== null && cd - (Date.now() - lastReport) > 0) {
                    ...
                    return await benny.reply(from, `Maaf, tetapi kamu telah mencapai limit menggunakan command ini.\nSilakan tunggu *${time.hours}* jam *${time.minutes}* menit *${time.seconds}* detik lagi.`, id)
                } else {
                    if (isGroupMsg) {
                        await benny.sendText(pilotNumber, `-----[ REPORT ]-----\n\n*From*: ${pushname}\n*ID*: ${sender.id}\n*Group*: ${(name || formattedTitle)}\n*Message*: ${q}`)
                        await benny.reply(from, `Halo ${pushname}!\nTerima kasih telah melapor, laporanmu akan kami segera terima.`, id)
                    } else {
                        await benny.sendText(pilotNumber, `-----[ REPORT ]-----\n\n*From*: 
${pushname}\n*ID*: ${sender.id}\n*Message*: ${q}`)
                        await benny.reply(from, `Halo ${pushname}!\nTerima kasih telah melapor, laporanmu akan kami segera terima.`, id)
	...
	case '#puisi':
	    if (isBanned) return await benny.reply(from, `Maaf nomor kamu telah dibanned!`, id)
			 ...
			 case '#shortlink':
			 if (isBanned) return await benny.reply(from, `Maaf nomor kamu telah dibanned!`, id)
            if (args.length == 0) return benny.reply(from, `ketik ${prefix}shortlink <url>`, id)
        ...
        if (args.length === 1) return benny.reply(from, 'Kirim perintah *#ytmp3 [linkYt]*, untuk contoh silahkan kirim perintah *#readme*')
            ...
            try {
                ...
                if (resp.error) {
                    ...
                    await benny.sendFileFromUrl(from, isLinks, `${title}.mp3`, '', id).catch(() => benny.reply(from, mess.error.Yt3, id))
                }
            } catch (err) {
                benny.sendText(pilotNumber, 'Error ytmp3 : '+ err)
		...
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n*#daftar* <nama | daerah>\n\nTanpa tanda <>`)
            ...
            try {
                ...
                if (ytv.error) {
                   ...
                   await benny.sendFileFromUrl(from, isLin, `${ytv.title}.mp4`, '', id).catch(() => benny.reply(from, mess.error.Yt4, id))
                }
            } catch (er) {
                benny.sendText(pilotNumber, 'Error ytmp4 : '+ er)
	...
	case '#tutorial':
		...
		if (!isRegistered) return await benny.reply(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`, id)
			...
			benny.sendImageAsSticker(from, tutor)
		    break
		case '#bot':
		if (isBanned) return await benny.reply(from, `Maaf, Nomor kamu telah dibanned!`, id)
		if (!isRegistered) return await benny.reply(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`, id)
	...
	case '#fb':
		if (isBanned) return await benny.reply(from, `Maaf nomor kamu telah dibanned!`, id)
            if (args.length === 1) return benny.reply(from, 'Kirim perintah *#fb [linkFb]* untuk contoh silahkan kirim perintah *#readme*', id)
            if (!args[1].includes('facebook.com')) return benny.reply(from, mess.error.Iv, id)
            benny.reply(from, mess.wait, id)
            const epbe = await fb(args[1])
            benny.sendFileFromUrl(from, epbe.url, `Cuih${epbe.exts}`, epbe.capt, id)
            break
    case '#owner':
    ...
    case '#anime':
		...
		if (!isRegistered) return await benny.reply(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`, id)
            ...
            if (animek.error) return benny.reply(from, animek.error, id)
            const res_animek = `${animek.result}\n\n${animek.sinopsis}`
            benny.sendFileFromUrl(from, animek.thumb, 'dewabatch.jpg', res_animek, id)
            break
	case '#anime2':
		if (isBanned) return await benny.reply(from, `Maaf nomor kamu telah dibanned!`, id)
		if (!isRegistered) return await benny.reply(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n#daftar`, id)
...
*Link*: ${url}`
	...
	/* case '#ttp':
            ...
            break */
	case '#ttp2':
            if (args.length === 1) return benny.reply(from, `Kirim perintah *#ttp2 [ Teks ]*, contoh *#ttp2 Benny*`, id)
            const ttp2t = body.slice(6)
            const lttp2 = ["Orange","White","Green","Black","Purple","Red","Yellow","Blue","Navy","Grey","Magenta","Brown","Gold"]
            const rttp2 = lttp2[Math.floor(Math.random() * (lttp2.length))]
            await benny.sendStickerfromUrl(from, `https://api.vhtear.com/textmaker?text=${ttp2t}&warna=${rttp2}&apikey=${vhtearkey}`)
            break
	case '#karakter':
	case '#karakter':
 	    const keywr = (args)
            ...
            if(find && find.id === groupId){
                ...
                if(cekuser && isIn !== false){
                    if(isKasar){
                        ...
                        if(denda){
                            await benny.reply(from, "Jangan badword bodoh\nDenda +5.000\nTotal : Rp"+formatin(denda.denda), id)
                        }
                    }
                } else {
