...
module.exports = msgHandler = async (benny, message) => {
    try {
        ...
        case '#donate':
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n*#daftar* <nama | daerah>\n\nTanpa tanda <>`)
            benny.sendLinkWithAutoPreview(from, `https://youtube.com/c/bennyhidayat`)
            break
        case '#tts':
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n*#daftar* <nama | daerah>\n\nTanpa tanda <>`)
		...
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n*#daftar* <nama | daerah>\n\nTanpa tanda <>`)
            ...
            .on('exit', () => {
                benny.sendImage(from, './media/img/after.jpg', 'nulis.jpg', 'Nih mhank', id)
            })
            break
			case '#play':
			if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n*#daftar* <nama | daerah>\n\nTanpa tanda <>`)
            if (args.length == 0) return benny.reply(from, `Untuk mencari lagu dari youtube\n\nPenggunaan: #play judul lagu`, id)
            axios.get(`https://arugaytdl.herokuapp.com/search?q=${body.slice(6)}`)
            .then(async (res) => {
                await benny.sendFileFromUrl(from, `${res.data[0].thumbnail}`, ``, `Lagu ditemukan\n\nJudul: ${res.data[0].title}\nDurasi: ${res.data[0].duration}detik\nUploaded: ${res.data[0].uploadDate}\nView: ${res.data[0].viewCount}\n\nsedang dikirim`, id)
				rugaapi.ytmp3(`https://youtu.be/${res.data[0].id}`)
				.then(async(res) => {
					if (res.status == 'error') return benny.sendFileFromUrl(from, `${res.link}`, '', `${res.error}`)
					await benny.sendFileFromUrl(from, `${res.thumb}`, '', `Lagu ditemukan\n\nJudul ${res.title}\n\nSabar lagi dikirim`, id)
					await benny.sendFileFromUrl(from, `${res.link}`, '', '', id)
					.catch(() => {
						benny.reply(from, `URL INI ${args[0]} SUDAH PERNAH DI DOWNLOAD SEBELUMNYA ..URL AKAN RESET SETELAH 60 MENIT`, id)
					})
				})
            })
            .catch(() => {
		...
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n*#daftar* <nama | daerah>\n\nTanpa tanda <>`)
            ...
            try {
                ...
                benny.sendText(ownerNumber, 'Error ytmp3 : '+ err)
		...
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n*#daftar* <nama | daerah>\n\nTanpa tanda <>`)
            ...
            try {
                ...
                benny.sendText(ownerNumber, 'Error ytmp4 : '+ er)
		...
		/*case '#lapor':
		case '#report':
		benny.sendMessageTo('62822119995947@c.us')
		break*/
        case '#cuaca':
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n*#daftar* <nama | daerah>\n\nTanpa tanda <>`)
            if (args.length === 1) return benny.reply(from, 'Kirim perintah *#cuaca [tempat]*\nContoh : *#cuaca tangerang', id)
            const tempat = body.slice(7)
            const weather = await get.get('https://api-zefian.glitch.me/api/cuaca?q='+ tempat).json()
            if (weather.error) {
                benny.reply(from, weather.error, id)
            } else {
                benny.reply(from, `➸ tempat : ${weather.result.tempat}\n\n➸ Angin : ${weather.result.angin}\n➸ Cuaca : ${weather.result.cuaca}\n➸ Deskripsi : ${weather.result.desk}\n➸ Kelembapan : ${weather.result.kelembapan}\n➸ Suhu : ${weather.result.suhu}\n➸ Udara : ${weather.result.udara}`, id)
            }
            break
        case '#fb':
            ...
            break
        case '#owner':
            benny.sendContact(from, '6289636006352@c.us')
            break
        case '#ig':
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n*#daftar* <nama | daerah>\n\nTanpa tanda <>`)
		...
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n*#daftar* <nama | daerah>\n\nTanpa tanda <>`)
            if (!isPilot) return
            //if (isGroupMsg) return benny.reply(from, 'Sorry this command for private chat only!', id)
            if (args.length === 2) {
		...
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n*#daftar* <nama | daerah>\n\nTanpa tanda <>`)
            ...
            for (let ids of chatz) {
                ...
                if (!cvk.isReadOnly) await benny.sendText(ids, `[ BENNY BOT Broadcast ]\n\n${msg}`)
            }
...
if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n*#daftar* <nama | daerah>\n\nTanpa tanda <>`)
 ...
 await benny.reply(from, 'Selamat nomor anda telah terdaftar sebagai premium user gunakan fitur premium dengan bijak. \n Terima Kasih', id)
   ...
   case '#info':
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n*#daftar* <nama | daerah>\n\nTanpa tanda <>`)
            benny.sendLinkWithAutoPreview(from, 'https://github.com/bennyganteng/BennyBOT', info)
            break
        case '#snk':
		if (!isRegistered) return benny.sendText(from, `Nomor kamu belum terdafar! \n\nSilahkan register dengan format:\n*#daftar* <nama | daerah>\n\nTanpa tanda <>`)
